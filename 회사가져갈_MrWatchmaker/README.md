...
## MrWatchmaker modifications

This repository is based on the open-source project `tg` (vacaboja/tg). The upstream license is the GNU GPL v2, and the full license text is included in `LICENSE`.

In addition to the upstream `tg` code, this repository contains MrWatchmaker-specific modifications, branding, and build/packaging changes for the "MrWatchmaker" product.

MrWatchmaker-specific modifications include:
- Source code updates in `src/`
- MrWatchmaker-specific patch/fix utilities (e.g. `fix_*.py`, `patch_*.py`)
- Packaging/installer and build helpers under `packaging/` and related scripts
- Related documentation under `docs/` (where present)

Copyright (C) 2026 mrwatchmaker
Contact: +82-41-555-4171, phillcafe@gmail.com

MrWatchmaker-specific modifications are distributed under the terms of the GNU GPL v2.
