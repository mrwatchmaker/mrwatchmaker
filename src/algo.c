...
static void do_locate_events(int *events, struct processing_buffers *p, float *waveform, int last, int offset, int count)
{
	...
	for(i=0; i<count; i++) {
		int a = round(last - offset - i*p->period - 0.035*p->sample_rate);
		int b = round(last - offset - i*p->period + 0.035*p->sample_rate);
		if(a < 0 || b >= p->sample_count - p->period/2)
			events[i] = -1;
		else {
			int peak_idx;
			double max_val = vmax(p->tic_c, a, b+1, &peak_idx);
			events[i] = max_val > 0 ? offset + peak_idx : -1;
		}
	}
}
