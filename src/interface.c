...
static void handle_language_change(GtkMenuItem *menuitem, struct main_window *w)
{
	int lang_idx = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(menuitem), "lang_idx"));
	if (lang_idx != w->lang) {
		w->lang = lang_idx;
		save_on_change(w);
...
static void init_main_window(struct main_window *w)
{
	...
	// 창/작업표시줄 아이콘: 여러 크기를 제공해 스케일 시 찌그러짐 방지
	{
		const int icon_sizes[] = { 16, 32, 48, 64, 0 };
...
#ifdef _WIN32
		{
			...
			if (GetModuleFileNameA(NULL, exe, sizeof(exe)) > 0) {
				...
				if (slash) {
					*slash = '\0';
					snprintf(base, sizeof(base), "%s\\icons", exe);
				}
			}
		}
#endif
		for (int i = 0; icon_sizes[i] != 0; i++) {
			char path[600];
			snprintf(path, sizeof(path), "%s/%dx%d/apps/tg-timer.png",
				base, icon_sizes[i], icon_sizes[i]);
		...
		if (icon_list) {
			gtk_window_set_icon_list(GTK_WINDOW(w->window), icon_list);
			for (GList *l = icon_list; l; l = l->next)
				g_object_unref(l->data);
			g_list_free(icon_list);
		} else {
			GdkPixbuf *fallback = gdk_pixbuf_new_from_file("icons/48x48/apps/tg-timer.png", NULL);
			if (fallback) {
	...
	// ── 전체 다크 테마 CSS ────────────────────────────────────────────
	{
		...
		const char *dark_css =
			"* { color: #d0d8e8; }\n"
			"window, .background { background-color: #12121e; }\n"
			/* 툴바 (상단용 - 현재 미사용) */
			".toolbar-box { background-color: #0d0f1a;"
			"  border-bottom: 2px solid #c8a840; padding: 7px 14px; }\n"
			/* 툴바 (하단용) */
			".toolbar-bottom { background-color: #0d0f1a;"
			"  border-top: 2px solid #c8a840; padding: 7px 14px; }\n"
			".toolbar-sep { background-color: #2a3350;"
			"  min-width: 1px; margin: 0 8px; }\n"
			".ctrl-label { color: #6a7a9a; font-size: 11px; font-weight: bold;"
			"  letter-spacing: 1px; margin-bottom: 1px; }\n"
			".brand-label { color: #c8a840; font-size: 17px; font-weight: bold;"
			"  letter-spacing: 3px; }\n"
			/* 버튼 */
			"button { background-image: none; background-color: #1c2038;"
			"  color: #c8d4e8; border-radius: 6px;"
			"  border: 1px solid #2e3a52; padding: 4px 10px; }\n"
			"button:hover { background-color: #252d4a; border-color: #c8a840; color: #f0e0a0; }\n"
			"button:active { background-color: #151a30; }\n"
			/* entry / spinbutton */
			"entry { background-color: #181c2e; color: #d8e4f0;"
			"  border: 1px solid #2e3a52; border-radius: 5px;"
			"  font-size: 13px; min-height: 28px; caret-color: #c8a840; }\n"
			"entry:focus { border-color: #c8a840; box-shadow: none; }\n"
			"entry selection { background-color: #c8a840; color: #0d0f1a; }\n"
			"spinbutton { background-color: #181c2e; color: #d8e4f0;"
			"  border: 1px solid #2e3a52; border-radius: 5px;"
			"  font-size: 13px; min-height: 28px; }\n"
			"spinbutton:focus { border-color: #c8a840; }\n"
			"spinbutton > entry { background-color: transparent;"
			"  border: none; color: #d8e4f0; }\n"
			"spinbutton button { background-color: transparent; border: none;"
			"  color: #c8a840; padding: 0 3px; min-height: 0; }\n"
			/* combobox */
			"combobox > box > entry { background-color: #181c2e; color: #d8e4f0;"
			"  border: 1px solid #2e3a52; border-right: none;"
			"  border-radius: 5px 0 0 5px; font-size: 13px; min-height: 28px; }\n"
			"combobox > box > entry:focus { border-color: #c8a840; }\n"
			"combobox > box > button { background-image: none;"
			"  background-color: #1c2038; color: #c8a840;"
			"  border: 1px solid #2e3a52; border-left: none;"
			"  border-radius: 0 5px 5px 0; padding: 0 6px; }\n"
			"combobox > box > button:hover { background-color: #252d4a; }\n"
			/* 노트북 */
			"notebook > header { background-color: #0d0f1a;"
			"  border-bottom: 1px solid #2a3350; }\n"
			"notebook > header > tabs > tab { background-color: #0d0f1a;"
			"  color: #506070; padding: 5px 16px;"
			"  border-radius: 6px 6px 0 0; }\n"
			"notebook > header > tabs > tab:checked { background-color: #1c2038;"
			"  color: #c8a840; font-weight: bold; }\n"
			/* 프레임 테두리 */
			"frame { border-color: #2a3248; }\n"
			"frame > border { border-color: #2a3248; background-color: #2a3248; }\n"
			"frame box { background-color: #12121e; }\n"
			/* 스크롤바 */
			"scrollbar { background-color: #0d0f1a; }\n"
			"scrollbar slider { background-color: #2a3248; border-radius: 4px;"
			"  min-width: 6px; min-height: 6px; }\n"
			"scrollbar slider:hover { background-color: #c8a840; }\n"
			/* 메뉴 */
			"menu { background-color: #181c2e; border: 1px solid #2e3a52; }\n"
			"menuitem { background-color: #181c2e; color: #c0cce0; padding: 5px 14px; }\n"
			"menuitem:hover { background-color: #252d4a; color: #c8a840; }\n"
			"checkmenuitem:checked { color: #c8a840; }\n"
			"separator { background-color: #2e3a52; min-height: 1px; }\n"
			"menubutton > button { background-color: transparent; border: none; }\n"
			"menubutton > button:hover { background-color: #1c2038;"
			"  border: 1px solid #2e3a52; }\n";
		gtk_css_provider_load_from_data(css, dark_css, -1, NULL);
	...
	// ── 커스텀 툴바 ──────────────────────────────────────────────────
	GtkWidget *toolbar_eb = gtk_event_box_new();
	gtk_style_context_add_class(gtk_widget_get_style_context(toolbar_eb), "toolbar-box");
	// 툴바는 notebook 배치 후 맨 아래에 추가 (아래에서 pack_end)

	GtkWidget *hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 8);
	gtk_container_set_border_width(GTK_CONTAINER(hbox), 0);
	gtk_container_add(GTK_CONTAINER(toolbar_eb), hbox);

	// 브랜드 로고
	GtkWidget *brand = gtk_label_new(_("⚙ MrWatchmaker"));
	gtk_style_context_add_class(gtk_widget_get_style_context(brand), "brand-label");
	gtk_box_pack_start(GTK_BOX(hbox), brand, FALSE, FALSE, 4);

	// 구분선
	GtkWidget *sep1 = gtk_separator_new(GTK_ORIENTATION_VERTICAL);
	gtk_style_context_add_class(gtk_widget_get_style_context(sep1), "toolbar-sep");
	gtk_box_pack_start(GTK_BOX(hbox), sep1, FALSE, FALSE, 0);

	// BPH 그룹
	{
		GtkWidget *grp = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
		GtkWidget *lbl = gtk_label_new(_("BPH"));
		gtk_style_context_add_class(gtk_widget_get_style_context(lbl), "ctrl-label");
		...
		gtk_box_pack_start(GTK_BOX(grp), lbl, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(grp), w->bph_combo_box, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(hbox), grp, FALSE, FALSE, 0);
	}

	// 구분선
	GtkWidget *sep2 = gtk_separator_new(GTK_ORIENTATION_VERTICAL);
	gtk_style_context_add_class(gtk_widget_get_style_context(sep2), "toolbar-sep");
	gtk_box_pack_start(GTK_BOX(hbox), sep2, FALSE, FALSE, 0);

	// Lift Angle 그룹
	{
		GtkWidget *grp = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
		GtkWidget *lbl = gtk_label_new(_("LIFT ANGLE"));
		gtk_style_context_add_class(gtk_widget_get_style_context(lbl), "ctrl-label");
		gtk_widget_set_halign(lbl, GTK_ALIGN_START);
		...
		gtk_box_pack_start(GTK_BOX(grp), lbl, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(grp), w->la_spin_button, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(hbox), grp, FALSE, FALSE, 0);
	}

	// 구분선
	GtkWidget *sep3 = gtk_separator_new(GTK_ORIENTATION_VERTICAL);
	gtk_style_context_add_class(gtk_widget_get_style_context(sep3), "toolbar-sep");
	gtk_box_pack_start(GTK_BOX(hbox), sep3, FALSE, FALSE, 0);

	// Calibration 그룹
	{
		GtkWidget *grp = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
		GtkWidget *lbl = gtk_label_new(_("CAL"));
		gtk_style_context_add_class(gtk_widget_get_style_context(lbl), "ctrl-label");
		gtk_widget_set_halign(lbl, GTK_ALIGN_START);
		...
		gtk_box_pack_start(GTK_BOX(grp), lbl, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(grp), w->cal_spin_button, FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(hbox), grp, FALSE, FALSE, 0);
	}
	...
	// Snapshot 버튼
	w->snapshot_button = gtk_button_new_with_label(_("📸  Snapshot"));
	...
	// Snapshot 이름 필드
	GtkWidget *name_label = gtk_label_new(_("Snapshot:"));
	w->snapshot_name_entry = gtk_entry_new();
	w->snapshot_name = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
	...
	// ... Light checkbox
	GtkWidget *light_checkbox = gtk_check_menu_item_new_with_label(_("Light algorithm"));
	...
	// Language Selector
	gtk_menu_shell_append(GTK_MENU_SHELL(command_menu), gtk_separator_menu_item_new());
	GtkWidget *lang_item = gtk_menu_item_new_with_label(_("Language / 언어"));
	GtkWidget *lang_submenu = gtk_menu_new();
	gtk_menu_item_set_submenu(GTK_MENU_ITEM(lang_item), lang_submenu);
	
	for (int i = 0; i < i18n_get_num_langs(); i++) {
		GtkWidget *li = gtk_menu_item_new_with_label(i18n_get_lang_name(i));
		g_object_set_data(G_OBJECT(li), "lang_idx", GINT_TO_POINTER(i));
		g_signal_connect(li, "activate", G_CALLBACK(handle_language_change), w);
		gtk_menu_shell_append(GTK_MENU_SHELL(lang_submenu), li);
	}
	gtk_menu_shell_append(GTK_MENU_SHELL(command_menu), lang_item);
	gtk_menu_shell_append(GTK_MENU_SHELL(command_menu), gtk_separator_menu_item_new());
	...
	// 툴바는 맨 아래 (상단 테두리로 구분)
	gtk_style_context_remove_class(gtk_widget_get_style_context(toolbar_eb), "toolbar-box");
	gtk_style_context_add_class(gtk_widget_get_style_context(toolbar_eb), "toolbar-bottom");
	gtk_box_pack_end(GTK_BOX(vbox), toolbar_eb, FALSE, FALSE, 0);
...
int main(int argc, char **argv)
{
	...
	GtkApplication *app = gtk_application_new ("com.mrwatchmaker.app", G_APPLICATION_HANDLES_OPEN);
